...
import config, db, encryption, utils, pyotp
import traceback # Keep for potential future debugging

app = Flask(__name__)
app.secret_key = config.SECRET_KEY
if not app.secret_key: raise ValueError("FLASK_SECRET_KEY is not set")
...
def login():
    ...
    if request.method == 'POST':
        ...
        if not username or not password: flash('Username and password are required.', 'error'); return render_template('login.html')
        ...
        if user_data and encryption.verify_master_password(user_data['password_hash'], password):
            ...
            if user_data.get('is_2fa_enabled'):
                ...
                try:
                    ...
                    session['encryption_key'] = key; session['is_2fa_enabled'] = False
...
def login_2fa():
    if '_2fa_user_id' not in session: flash('Please login first.', 'warning'); return redirect(url_for('login'))
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            flash('Login successful!', 'success'); return redirect(url_for('vault'))
        except Exception as e: flash(f'Key derivation failed: {e}', 'error'); session.clear(); return redirect(url_for('login'))
    ...
    if request.method == 'POST':
        ...
        if error: flash(error, 'error')
            ...
            try:
                ...
                if user_id: flash('Account created! Please log in.', 'success'); try: db.ensure_indexes()
                except Exception as idx_e: print(f"Warn: Signup index err: {idx_e}"); return redirect(url_for('login'))
    ...
    if request.method == 'POST':
        ...
        if totp.verify(totp_code, valid_window=1):
            if db.set_user_2fa_secret(user_id, secret_key) and db.enable_user_2fa(user_id, enable=True):
                flash('2FA enabled!', 'success'); session['is_2fa_enabled'] = True; return redirect(url_for('vault'))
            else: flash('DB error saving 2FA settings.', 'error'); return redirect(url_for('setup_2fa'))
        else:
...
def add_entry():
    ...
    if not laptop_server or not entry_username or not password: flash('All fields required.', 'error')
    elif not encryption_key: flash('Session key error.', 'error'); session.clear(); return redirect(url_for('login'))
    else:
        try:
...
def generate_password_api():
    ...
    except Exception as e: print(f"GenPass API Error: {e}"); return jsonify({'error': 'Failed to generate'}), 500

@app.route('/get_password/<entry_id>')
@login_required
def get_password_api(entry_id):
    user_id = session['user_id']; encryption_key = session.get('encryption_key')
    if not encryption_key: return jsonify({'error': 'Session key missing'}), 401
    try:
