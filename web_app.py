...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        needs_key = request.endpoint not in ['login', 'signup', 'logout', 'login_2fa', 'index', 'setup_2fa', 'disable_2fa', 'admin_users', 'admin_toggle_user_status', 'admin_change_user_role', 'admin_delete_user', 'static']
...
def login():
    ...
    if request.method == 'POST':
        ...
        if not username or not password: flash('Username and password required.', 'error'); return render_template('quantum_login_v2.html') # Use V3 name if created
        user_data = db.find_user(username)
        if user_data and encryption.verify_master_password(user_data['password_hash'], password):
             if not user_data.get('is_active', True): flash('Account disabled.', 'error'); return render_template('quantum_login_v2.html') # Use V3 name if created
             ...
             if user_data.get('is_2fa_enabled'):
                 ...
                 except Exception as e: print(f"DEBUG Login Err (no 2FA): {e}"); traceback.print_exc(); flash(f'Login process error: {e}', 'error'); session.clear(); return render_template('quantum_login_v2.html') # Use V3 name if created
    ...
    return render_template('quantum_login_v2.html') # Use V3 name if created
...
def login_2fa():
    ...
    if request.method == 'POST':
        ...
        if not password or not totp_code: flash('Password and code required.', 'error'); return render_template('quantum_login_2fa_v2.html') # Use V3 name if created
        ...
        if not encryption.verify_master_password(user_data['password_hash'], password): flash('Invalid password.', 'error'); return render_template('quantum_login_2fa_v2.html') # Use V3 name if created
        totp_secret = user_data.get('totp_secret')
        if not totp_secret: flash('2FA secret not found.', 'error'); session.clear(); return redirect(url_for('login'))
        totp = pyotp.TOTP(totp_secret)
        if not totp.verify(totp_code, valid_window=1): flash('Invalid authenticator code.', 'error'); return render_template('quantum_login_2fa_v2.html') # Use V3 name if created
        try:
            key = encryption.derive_key(password, user_data['salt'])
            session['user_id'] = session.pop('_2fa_user_id'); session['username'] = session.pop('_2fa_username'); session['salt'] = session.pop('_2fa_salt'); session['role'] = session.pop('_2fa_role')
    ...
    return render_template('quantum_login_2fa_v2.html') # Use V3 name if created
...
def signup():
    ...
    return render_template('quantum_signup_v2.html') # Use V3 name if created
...
def setup_2fa():
    ...
    if request.method == 'POST':
        ...
        if totp.verify(totp_code, valid_window=1):
            ...
            return render_template('quantum_setup_2fa_v2.html', secret_key=secret_key, qr_code_data=qr_code_data) # Use V3 name if created
...
@admin_required
def admin_users():
    all_users = db.get_all_users(); return render_template('quantum_admin_users.html', users=all_users) # Use V3 name if created
...
def admin_view_user_vault(user_id):
     ...
     entries = db.get_vault_entries_for_user(user_id); return render_template('quantum_admin_view_vault.html', entries=entries, target_user=target_user) # Use V3 name if created
...
def vault():
    ...
    return render_template('quantum_vault_v2.html', entries=entries, search_term=search_term, is_2fa_enabled=session.get('is_2fa_enabled'), current_username=session.get('username')) # Use V3 name if created
...
def add_entry():
    ...
    if not laptop_server or not entry_username or not password:
        flash('Laptop/Server ID, Username, Password required.', 'error')
    elif not encryption_key:
        flash('Session error: Key missing.', 'error'); session.clear(); return redirect(url_for('login'))
    else:
...
# Edit route redirects to vault, JS handles modal now
@app.route('/edit_entry/<entry_id>', methods=['GET'])
@login_required
def edit_entry(entry_id):
    # flash("Use the edit icon on the vault dashboard.", "info") # Optional info message
    return redirect(url_for('vault'))

# Handles POST from Edit Modal
...
# Handles POST from Delete buttons
...
# API to get password only (for copy/show in vault card)
@app.route('/get_password/<entry_id>')
@login_required
def get_password_api(entry_id):
    user_id = session['user_id']; encryption_key = session.get('encryption_key')
    if not encryption_key: return jsonify({'error': 'Key missing'}), 401
    try: entry_data = db.find_entry_by_id_and_user(entry_id, user_id); # Checks ownership
        if entry_data: encrypted_pass = entry_data.get('encrypted_password');
             if encrypted_pass: decrypted_pass = encryption.decrypt_data(encrypted_pass, encryption_key);
...
# API to get full details for editing modal
...
def get_entry_details_api(entry_id):
    ...
    try: entry_data = db.find_entry_by_id_and_user(entry_id, user_id); # Checks ownership
        ...
        if encrypted_pass: decrypted_password = encryption.decrypt_data(encrypted_pass, encryption_key) or ""
        details = {'laptop_server': entry_data.get('laptop_server', ''), 'brand_label': entry_data.get('brand_label', ''), 'entry_username': entry_data.get('entry_username', ''), 'password': decrypted_password}
...
if __name__ == '__main__':
    try: print("Attempting initial database connection check..."); db_conn_check = db.connect_db();
