...
document.addEventListener('DOMContentLoaded', function() {
    console.log("Quantum Vault V3 JS Loaded - Fixing Generate/Cancel");
    ...
    } else { /* console errors if elements not found */ }


    // --- Add Entry Form Interactions ---
    const addEntryForm = document.getElementById('add-entry-form');
    if (addEntryForm) {
        const generateBtnAdd = addEntryForm.querySelector('#generate-btn'); // Correct ID for Add form button
        const passwordFieldAdd = addEntryForm.querySelector('#add_entry_password'); // Use CORRECT ID for Add form password input
        const showHideBtnAdd = addEntryForm.querySelector('#show-hide-add-btn'); // Correct ID for Add form show/hide
        const cancelBtnAdd = addEntryForm.querySelector('#cancel-add-entry'); // Correct ID for Add form cancel
             ...
             generateBtnAdd.addEventListener('click', async function() {
                ...
                try {
                    ...
                    if (showHideBtnAdd) { // Update corresponding show/hide button
                        const icon = showHideBtnAdd.querySelector('i'); if(icon) icon.className = 'bi bi-eye-slash-fill'; showHideBtnAdd.title = 'Hide Password';
                    }
        ...
        } else { if (!generateBtnAdd) console.error("JS Error: #generate-btn not found in Add Form"); if (!passwordFieldAdd) console.error("JS Error: #add_entry_password not found in Add Form"); }
    ...
    if (editEntryModalElement) {
        ...
        const editPasswordInput = document.getElementById('modal_entry_password'); // Correct ID for modal password
        ...
        const generateEditModalBtn = document.getElementById('generate-modal-btn'); // Correct ID for modal generate
        const showHideEditModalBtn = document.getElementById('show-hide-modal-btn'); // Correct ID for modal show/hide
        ...
        document.querySelectorAll('.entry-card .edit-btn').forEach(button => {
            button.addEventListener('click', async function(event) {
                 // ... (Modal population logic - unchanged from previous correct version) ...
                ...
                try { const data = await fetchApi(`/get_entry_details/${entryId}`);
                    editLaptopServerInput.value = data.laptop_server || ''; editBrandLabelInput.value = data.brand_label || ''; editUsernameInput.value = data.entry_username || ''; editPasswordInput.value = data.password || ''; editPasswordInput.placeholder = "Leave blank to keep current"; editPasswordInput.type = 'password';
         ...
         editEntryModalElement.addEventListener('hidden.bs.modal', function (event) { /* ... modal reset logic ... */ editForm.reset(); editModalTitle.textContent = 'Edit Vault Entry'; editPasswordInput.required = false; editForm.action = '#'; if(editModalHelpText) editModalHelpText.style.display = 'none'; });
