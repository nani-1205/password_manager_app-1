    ...
    if _client is None:
        try:
            ...
            if config.mongo_user:
                connection_args.update({ # Indented under 'if'
            ...
            # Create the MongoDB client instance (aligned with 'if config.mongo_user')
        ...
        # --- Error Handling for Connection (aligned with 'try') ---
            ...
            _client = None; _db = None; raise # Re-raise
    # Return the database object (aligned with 'if _client is None:')
    return _db
...
def add_user(username, hashed_password, salt, role='user', is_active=True):
    ...
    try:
        ...
        if user_count == 0:
             user_data["role"] = "admin"; print("INFO: First user set as admin.")
...
def get_vault_entries_for_user(target_user_id): # Admin view metadata
     ...
     try:
        ...
        entries = list(db_conn[config.VAULT_COLLECTION].find(
            {"user_id": query_user_id}, {"encrypted_password": 0}
        ).sort([("brand_label", 1), ("laptop_server", 1)])); return entries
     except Exception as e:
...
def find_entry_by_id(entry_id_str): # Finds entry regardless of owner
    """Finds a single vault entry by its ID (does not check ownership)."""
    try:
        db_conn = get_db(); entry_obj_id = ObjectId(entry_id_str)
        entry = db_conn[config.VAULT_COLLECTION].find_one({"_id": entry_obj_id}); return entry
    except Exception as e:
