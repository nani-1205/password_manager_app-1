...
def find_user(username):
    try:
        db_conn = get_db();
        user_data = db_conn[config.USERS_COLLECTION].find_one({"username": username},{"_id": 1, "username": 1, "password_hash": 1, "salt": 1, "totp_secret": 1, "is_2fa_enabled": 1, "role": 1, "is_active": 1})
        return user_data
...
def add_user(username, hashed_password, salt, role='user', is_active=True):
    try:
        db_conn = get_db(); user_data = {"username": username, "password_hash": hashed_password, "salt": salt, "totp_secret": None, "is_2fa_enabled": False, "role": role, "is_active": is_active}
        user_count = db_conn[config.USERS_COLLECTION].count_documents({})
        if user_count == 0: user_data["role"] = "admin"; print("INFO: First user set as admin.")
...
def get_vault_entries(user_id, search_term=None): # For user's own vault
    try:
        db_conn = get_db(); query_user_id = ObjectId(user_id) if isinstance(user_id, str) else user_id; query = {"user_id": query_user_id}
...
def get_vault_entries_for_user(target_user_id): # Admin view metadata
     try:
        ...
        entries = list(db_conn[config.VAULT_COLLECTION].find({"user_id": query_user_id}, {"encrypted_password": 0}).sort([("brand_label", 1), ("laptop_server", 1)])); return entries
...
def update_vault_entry(entry_id, laptop_server, brand_label, entry_username, encrypted_password): # Assumes owner check done
    try:
