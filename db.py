...
# --- connect_db, get_db, close_db (Unchanged) ---
def connect_db():
    global _client, _db
    if _client is None:
        try:
...
def ensure_indexes():
    ...
    try:
        ...
        if db_conn:
            ...
            # Compound index including new field
...
# --- Vault Operations ---
# UPDATE: add_vault_entry signature and data
...
# get_vault_entries - Modified slightly to potentially sort by new field too
def get_vault_entries(user_id, search_term=None):
    """Retrieves vault entries, optionally filtered and sorted."""
    try:
        db_conn = get_db()
        ...
        if search_term:
            ...
            # Search across multiple fields (optional enhancement)
            query["$or"] = [
            ...
            # Or just search laptop_server:
            # query["laptop_server"] = {"$regex": safe_search_term, "$options": "i"}

        # Sort results (e.g., by brand, then laptop/server ID)
...
# find_entry_by_id_and_user (Unchanged)
...
# update_vault_entry - Add brand_label parameter
def update_vault_entry(entry_id, laptop_server, brand_label, entry_username, encrypted_password):
    """Updates an existing vault entry including brand/label."""
    try:
        db_conn = get_db()
