...
def encrypt_data(data, key):
    ...
    if not data: return b'' # Handle empty strings or None
    if not isinstance(key, bytes): raise TypeError("Encryption key must be bytes.")

    try:
        # Correct indentation level 1 (relative to 'try')
        f = Fernet(key)
        # Ensure data is bytes before encrypting
        if isinstance(data, str):
            # Correct indentation level 2 (relative to 'try')
        ...
        # Correct indentation level 1 (relative to 'def')
...
def decrypt_data(encrypted_data, key):
    ...
    if not encrypted_data: return '' # Handle empty bytes or None
