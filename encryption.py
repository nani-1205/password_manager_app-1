...
def hash_master_password(password, salt):
    pwd_bytes = password.encode('utf-8'); return bcrypt.hashpw(pwd_bytes, salt)

def verify_master_password(stored_hash, provided_password):
    pwd_bytes = provided_password.encode('utf-8')
    if not isinstance(stored_hash, bytes): print("Error: Stored hash is not bytes."); return False
    try: return bcrypt.checkpw(pwd_bytes, stored_hash)
    except Exception as e: print(f"Error verifying password: {e}"); return False
